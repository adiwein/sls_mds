...
class TorchShape(Shape):
    def __init__(self, args, filename, device=None):
        Shape.__init__(self, args, filename)
        self.args = args
        if filename is not None:
            self.mass_mat, self.stiffness_mat = self.compute_laplacian()
            self.weights = torch.ones((self.size, self.size), dtype=torch.float64, device=device, requires_grad=True)
